...
PartidaGrafica::PartidaGrafica(scene::ISceneManager *smgr, Grafico::Skin *skin,
                               gui::IGUIEnvironment *env, int VelAnim)
    : Partida() {
  this->skin = skin;
  this->smgr = smgr;
  this->env = env;
  ...
  std::cout << "[D2] PartidaGrafica: parent=" << (void *)this->parent
            << std::endl;
  this->parent->setPosition(core::vector3df(0, 0, 0));
  this->t = new Tablero(smgr, this->skin, this->parent);
  std::cout << "[D2] PartidaGrafica: tablero=" << (void *)this->t
            << " size=" << this->t->getSize().X << "," << this->t->getSize().Y
            << "," << this->t->getSize().Z << std::endl;
...
bool PartidaGrafica::siguienteJugada() { return this->Siguiente(this->t); }
...
bool PartidaGrafica::MoverJugador(Reglas::Jugada &j, int idJugador) {
  ...
  if (this->en_curso)
    ju->Mover(
        smgr, p,
        this->t->getCeldasAMover(this->jugadores.at(idJugador)->getPosicion(),
                                 j.getPosicion()));
  else
    ju->setPosicion(p);
...
void PartidaGrafica::SetBarrera(Reglas::Jugada &j, int idJugador) {
  ...
  this->Barreras.push_back(
      new Barrera(smgr, this->skin, this->velAnim, this->t->getNodo()));
  unsigned int pos = this->Barreras.size();
  const std::vector<int> p = j.getPosicion();
  this->Barreras.at(pos - 1)->ColocaBarrera(this->t->getPosicionCelda(p), p,
                                            j.getDireccion(), this->smgr);
...
void PartidaGrafica::SetEscala(int x, int y, int z) {
  ...
  for (std::size_t i = 0; i < this->antorchas.size(); i++) {
    this->antorchas.at(i)->setEscalaAntorcha(this->escala.X, this->escala.Y,
                                             this->escala.Z);
  }
}

bool PartidaGrafica::SetJugadores(std::string rutaAgente1,
                                  std::string rutaAgente2) {
...
void PartidaGrafica::dropBarreras() {
  ...
  for (int i = 0; i < this->Barreras.size(); i++) {
    std::cout << "borrando la barrera : " << i << " de "
              << this->Barreras.size() << std::endl;
...
void PartidaGrafica::dropJugadores() {
  for (int i = 0; i < this->jugadores.size(); i++) {
    std::cout << "borrando el jugador: " << i << " de "
              << this->jugadores.size() << std::endl;
    Grafico::Jugador *ju = (Grafico::Jugador *)this->jugadores.at(i);
    delete (ju);
  }
