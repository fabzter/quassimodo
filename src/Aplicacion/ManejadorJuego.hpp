...
using namespace irr;
/**
 *Clase encargada de controlar todo el juego, desde aqui se puede iniciar una
 * nueva partida, eliminar los menus o volver a ellos. etc.
 */
class ManejadorJuego {
public:
  /**
   *Constructor de la clase, se encarga de inicializar los atributos y colocar
   * el menu princpal
   ...
   * @param grafico bandera que indica si la aplicacion correra en modo grafico
   * o solo en consola
   */
  ManejadorJuego(scene::ISceneManager *smgr, gui::IGUIEnvironment *env,
                 Grafico::Skin *skin, int VelAnim, bool grafico = true);
  /**
   ...
   * @return un caracter que representa la opcion seleccionada en modo consola,
   * o un caracter cualquiera si fue ejecutado en modo grafico
   */
  char setMenu();
  /**
   * elimina el menú principal y si se han seleccionado los agentes inicia la
   * partida
  ...
  void despachaJugada();
  /**
   * Pausa la partida si es que está en curso, O pone la partida a correr.,
   * manda a llamar al manejador de GUI para que haga los cambios
...
private:
  ...
  bool hayagente;
  /**
   * Un apuntador a un objeto de la clase ManejadorGUI el cual maneja la
   * interaccion con el usuario
   */
  Grafico::ManejadorGUI *mgui;
  /**
   * Un apuntador de la clase Partida que es la que se encarga de mantener la
   * partida
   */
  Partida *partida;
  /**
   * Un apuntador a un objeto de la clase Grafico::skin que tiene los modelos y
   * las texturas del programa
   */
  Grafico::Skin *skin;

  /**
   * Un apuntador a un nodo de terreno que es donde se carga y amacena nuestro
   * piso del juego
   */
