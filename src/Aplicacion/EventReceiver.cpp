...
bool EventReceiver::OnEvent(const SEvent &event) {
  ...
  // revisa si el evento fue un evento del teclado, y si es así se modifica el
  // estado de la tecla presionada
  ...
  case irr::EET_KEY_INPUT_EVENT:
    ...
    break;
    // si el elento fue un evento del mouse almecanamos la posicion de éste, y
    // si presiono el botón zquierdo o no.
  ...
  case irr::EET_GUI_EVENT:
    ...
    if (event.GUIEvent.EventType == gui::EGET_BUTTON_CLICKED) {
      std::cout << "[D2] Button clicked: id=" << event.GUIEvent.Caller->getID()
                << std::endl;
    ...
    if (event.GUIEvent.EventType == gui::EGET_FILE_SELECTED) {
      this->app->getManJuego()->setAgente(
          this->app->getManJuego()->getManejadorGUI()->getPath(), this->noA);
