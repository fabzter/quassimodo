...
| **D2.1 — Minimal graphical match** | Lean argv-driven graphical match (`aplicacion agentA.py agentB.py`): IrrlichtMt window, textured 3D board + 81 cells + two pawns + static torches, full AI-vs-AI match to "Hay un ganador!". Menu/animation/GUI deferred (KB-D2-001..006). | ✅ Done | [plan](plans/2026-05-30-phase-d2.1-graphical-match.md), **[PR #8](https://github.com/fabzter/quassimodo/pull/8) (merged)** |
| **D2.2a·M1 — Movement & framing** | Re-implement the deleted animator subsystem as a general `Animador` (tween manager) + eager command/event queue (`EventoJugada`) + dt driver loop: animated pawn jumps, restored move-pacing, static 3/4 camera behind a `CamaraController` seam. Match plays to "Hay un ganador!" with a pawn verifiably caught mid-jump (vision-confirmed); barrier slide (`DeslizarAnim`) implemented but unexercised by the Camina gate. consola regression intact; no `boost_python`. | ✅ Done | [spec](specs/2026-06-02-d2.2a-animation-camera-atmosphere-design.md), [plan](plans/2026-06-02-phase-d2.2a-m1-movement-framing.md), **PR #9** |
| **D2.2a·M2 — Atmosphere & polish** | Billboard torch flames (KB-D2-001), blob piece shadows (KB-D2-002), ground+sky backdrop, board-base centering + debug-`cout` strip (KB-D2-005). First-pass visuals, expect vision-review iteration. | ⬜ **NEXT** | plan: TBD (after M1) |
| **D2.2b — Interactivity** | Rebuild menu/GUI + message boxes from surviving primitives; re-include `EventReceiver` input; interactive camera controls (orbit/pan/zoom via the `CamaraController` seam). Re-port `Aplicacion`/`ManejadorJuego`. (KB-D2-003.) | ⬜ Planned | plan: TBD |
