...
| **D2.2a·M1 — Movement & framing** | Re-implement the deleted animator subsystem as a general `Animador` (tween manager) + eager command/event queue (`EventoJugada`) + dt driver loop: animated pawn jumps + barrier slides, restored move-pacing, static 3/4 camera behind a `CamaraController` seam. **Spec + full 7-task plan written & committed; NOT yet executed.** | ⬜ **NEXT (execute the plan)** | [spec](specs/2026-06-02-d2.2a-animation-camera-atmosphere-design.md), [plan](plans/2026-06-02-phase-d2.2a-m1-movement-framing.md) |
| **D2.2a·M2 — Atmosphere & polish** | Billboard torch flames (KB-D2-001), blob piece shadows (KB-D2-002), ground+sky backdrop, board-base centering + debug-`cout` strip (KB-D2-005). First-pass visuals, expect vision-review iteration. | ⬜ Planned | plan: TBD (after M1) |
