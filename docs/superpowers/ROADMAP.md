...
| **D1.5 — pybind11 spike** | De-risk replacing Boost.Python with pybind11: trampoline override (`PYBIND11_OVERRIDE_PURE`), STL conversions (`pybind11/stl.h`), embedding + cross-`.so` sharing (`pybind11/embed.h`). | ⬜ **NEXT** | plan: TBD (`spikes/pybind/`) |
